...
// TestClinic_InviteDoctorToClinic_BeyondBaseIncluded_StillAllowed confirma
// que el plan base (billing.ClinicBaseIncludedDoctors personas, dueño
// incluido) no es un tope duro: se puede seguir invitando más allá de esa
// cantidad, y el excedente se refleja como "extraDoctors" (cobro extra
// automático vía syncClinicDoctorCount, ver GetClinic) en vez de
// rechazarse.
func TestClinic_InviteDoctorToClinic_BeyondBaseIncluded_StillAllowed(t *testing.T) {
	db := testutil.SetupTestDB(t)
	router := server.NewRouter(db)

	owner := testutil.CreateTestDoctor(t, db, "doc_clinic_owner_extra", "password123")
	clinic := models.Clinic{Name: "Clínica Grande", OwnerDoctorID: owner.ID, SubscriptionStatus: "active"}
	require.NoError(t, db.Create(&clinic).Error)
	require.NoError(t, db.Model(&owner).Updates(map[string]any{"clinic_id": clinic.ID, "is_clinic_owner": true}).Error)

	// El dueño ya es 1 de las billing.ClinicBaseIncludedDoctors (5) del plan
	// base — 4 miembros más completan el plan base sin costo extra.
	for i := 0; i < 4; i++ {
		member := testutil.CreateTestDoctor(t, db, fmt.Sprintf("doc_clinic_extra_member_%d", i), "password123")
		require.NoError(t, db.Model(&member).Update("clinic_id", clinic.ID).Error)
	}

	token := testutil.TokenFor(t, owner.ID, owner.Username)

	// Un 6° doctor (más allá del plan base) debe poder invitarse sin
	// problema — no hay tope, solo cobro extra una vez que acepte.

...
func TestSendDueAppointmentReminders_SendsForUpcomingWithinWindow(t *testing.T) {
	...
	workers.SendDueAppointmentReminders(db, sender.send)
...
// que no manda recordatorios para citas fuera de la ventana de 24h, para
// solicitudes públicas aún sin confirmar, ni para pacientes sin correo.
func TestSendDueAppointmentReminders_SkipsOutsideWindowOrWrongStatus(t *testing.T) {
	...
	patientNoEmail := models.Patient{FirstName: "Sin", LastName: "Correo", Phone: "5551112222"}
	require.NoError(t, db.Create(&patientNoEmail).Error)
	require.NoError(t, db.Create(&models.Appointment{
		PatientID: patientNoEmail.ID, DoctorID: doc.ID,
		AppointmentDateTime: time.Now().UTC().Add(5 * time.Hour), Status: "PENDING",
	}).Error)

	sender := &recordingSender{}
	workers.SendDueAppointmentReminders(db, sender.send)

	assert.Equal(t, 0, sender.count(), "ninguna de estas citas debía disparar un recordatorio")
}

// TestSendDueAppointmentReminders_RetriesAfterSendFailure confirma que si
// el envío falla (ej. SMTP no configurado), la cita se queda sin marcar
// para reintentarse en la siguiente pasada, en vez de darse por enviada.
func TestSendDueAppointmentReminders_RetriesAfterSendFailure(t *testing.T) {
	...
	workers.SendDueAppointmentReminders(db, failingSender.send)
	...
	workers.SendDueAppointmentReminders(db, workingSender.send)
